...
def draw_cloud(tws,wf):
	...
	wordcloud = WordCloud(font_path='simhei.ttf', max_font_size=40,\
		width=1000,height=700,background_color = 'black').generate_from_frequencies(freq)
	wordcloud.to_file('wc3.png')

def main():
    f1="D:\\code\\python\\HigherPython\\Week02\\stopwords_list.txt"
    f2="D:\\code\\python\\HigherPython\\Week02\\doc1.txt"
	stoppingwords=load_stoppping_words
	print(len(stoppingwords))
	ws=get_terms(f2,stoppingwords)
	print(len(ws))
